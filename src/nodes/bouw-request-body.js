...
const regels = [
  ...
  '',
  'REGELS:',
  '- Begin ALTIJD bij het dagprogramma (het blok met regels als "Dag 1: Johannesburg - Mpumalanga" of "Dag 11: Battlefields - Drakensbergen"): dat is de leidende bron voor welke stops er zijn, in welke volgorde, en op welke dagen. De overige reisinformatie gebruik je als aanvullende context per stop (streek, bezienswaardigheden, verblijfplaats), nooit om stops toe te voegen of te schrappen die het dagprogramma tegenspreken.',
  '- Neem alle steden, eilanden, parken en regio\'s op waar de reiziger overnacht of een substantieel deel van de dag doorbrengt',
  '- Kijk bij ELKE stop naar de volledige context van de dagbeschrijving (welke streek of provincie, welke bezienswaardigheden, wat de reiziger daar doet) voordat je destination, destination_geocode en country invult. Baseer je nooit alleen op de kopregel van een dag.',
  ...
  '- Let op: een vliegveld als Preveza telt zowel als aankomst (type "arrival", order 1) als als vertrekstop (type "transit", laatste order). Beide mogen in de route staan ook al zijn het dezelfde coördinaten. Sla de arrival-stop aan het BEGIN dus nooit over: de kaart moet altijd tonen waar de reis start.',
  ...
  '- Gebruik voor "destination" ALTIJD de commerciële, herkenbare naam waarmee de reistekst de plek aanprijst — dit is de naam die de reiziger op de kaart wil terugzien. Voorbeeld: noemt de tekst "River Kwai" terwijl het hotel bij Kanchanaburi of Sai Yok staat, dan is destination "River Kwai". Noemt de tekst de "Okavango Delta" of "Sossusvlei", dan is dat de destination. Zet dus NOOIT de technische overnachtingsplaats als destination wanneer de tekst een wervende gebieds- of bezienswaardigheidsnaam gebruikt; de vindbare plaatsnaam hoort uitsluitend in destination_geocode.',
  '- Als de reiziger verblijft in een LAND dat als uitstap binnen de rondreis wordt gepresenteerd (bijv. Lesotho of eSwatini tijdens een Zuid-Afrikareis), gebruik dan de landnaam als destination (bijv. "Lesotho") en de concrete verblijfplaats uit de tekst als destination_geocode (bijv. "Malealea"). Zo toont de kaart het land en staat de pin op de werkelijke verblijfplek. Sla zo\'n landverblijf NOOIT over.',
  '- Als de tekst een specifieke stad, wijk of dorp noemt als overnachtingsplek die binnen circa 30 kilometer van een grotere, bekendere bestemming ligt (bijv. Houtbaai bij Kaapstad), gebruik dan de grotere bestemming als destination én destination_geocode. Alleen als de verblijfplaats écht een eigen bestemming is op meer dan 30 kilometer afstand, neem je hem als aparte stop op. En: als de verblijfplaats een klein dorp op een eiland is dat al als bestemming telt (bijv. Agia Efimia op Kefalonia), gebruik dan de eilandnaam als destination. Verblijft de reiziger achtereenvolgens op MEERDERE plekken op één eiland dat als geheel wordt verkocht (bijv. Santo Antão met eerst Ponta do Sol en daarna Cruzinha), geef dan de EERSTE stop op het eiland de eilandnaam als destination (met de concrete verblijfplaats als destination_geocode) en geef de vervolgstops op datzelfde eiland hun eigen plaatsnaam als destination. Zo staat de eilandnaam altijd herkenbaar op de kaart.',
  ...
  '- Bij verblijf op een projectlocatie, lodge of opvangcentrum zonder eigen vindbare plaatsnaam (bijv. een vrijwilligersproject "midden in de jungle"): houd de project- of centrumnaam als destination (dat is de commerciële naam), en gebruik als destination_geocode de dichtstbijzijnde stad, plaats of streek die in de tekst wordt genoemd; wordt er geen plaats genoemd, gebruik dan de projectnaam plus het land.',
  '- DISAMBIGUATIE bij dubbelzinnige, kleine of natuurgebied-achtige plaatsnamen: als een plaatsnaam ook elders voorkomt, een klein/onbekend dorp is, of een nationaal park, natuurreservaat of streek betreft, voeg dan in destination_geocode (NIET in destination) de onderscheidende context toe die LETTERLIJK in de reistekst staat — bij voorkeur de provincie, regio of staat, of de bekende plaats of streek waar het gebied bij hoort. Formaat: "Plaats, Regio". Het veld destination blijft de korte weergavenaam voor op de kaart; destination_geocode dient uitsluitend om de juiste coördinaten op te zoeken.',
  '  Voorbeeld: de reistekst beschrijft een verblijf bij "Addo" in het Addo Elephant National Park, in de provincie Eastern Cape bij Gqeberha (Port Elizabeth). Zet dan destination: "Addo" en destination_geocode: "Addo, Eastern Cape". Zo belandt de pin niet in een verkeerde gelijknamige plaats elders in het land.',
  '  Verzin GEEN context die niet in de tekst staat. Gebruik alleen regio-, provincie- of parknamen die daadwerkelijk in de reisinformatie worden genoemd. Twijfel je, of is de plaats al wereldberoemd en eenduidig (bijv. Bangkok, Kaapstad, Chiang Mai)? Houd destination_geocode dan gelijk aan destination.',
  ...
  '- BELANGRIJK - vervoer over water: een auto kan NIET over open zee rijden. Als twee opeenvolgende stops op verschillende eilanden liggen, of door zee gescheiden zijn zonder vaste brug- of wegverbinding, dan is het vervoer "ferry" (of "flight" bij grote afstand over zee), NOOIT "car". Een eiland dat met een brug of dam aan het vasteland vastzit (bijvoorbeeld Lefkada) is wel per auto bereikbaar vanaf dat vasteland, maar tussen twee losse eilanden onderling vaar je altijd met de ferry. Voorbeeld: van Kefalonia naar Lefkada ga je per ferry, niet per auto.',
  ...
  '',
  'CONTROLEER JE ROUTE VOORDAT JE ANTWOORDT (dit voorkomt fouten):',
  '1. Vervoer vs. geografie: loop elke stop na en vraag je af of het opgegeven vervoer geografisch echt kan. Kan je deze afstand over land afleggen (auto/trein), of ligt er open zee tussen de twee stops? Verander elk "car" dat in werkelijkheid over zee gaat in "ferry" (of "flight" bij grote afstand). Let extra op eilanden onderling.',
  '2. Volgorde, start en terugkeer: klopt de reisvolgorde en volgt die het dagprogramma? Begint de route op de plaats waar de reiziger het reisland binnenkomt (als "arrival" wanneer daar niet wordt overnacht, anders als eerste "destination")? En keert een rondreis aan het einde terug naar de vertrekstad (als type "transit") voordat de internationale terugvlucht vertrekt?',
  '3. Geen verzonnen stops: staat elke stop ook echt als bezoek, stop of overnachting in de brontekst? Verwijder stops die je hebt afgeleid maar die er niet met zoveel woorden staan.',
  '4. Marker-namen: is elke destination de commerciële naam uit de tekst (River Kwai, Okavango Delta, Lesotho) en niet de technische overnachtingsplaats?',
  'Corrigeer de route op basis van deze controles en geef daarna pas het eindresultaat.',
  '',
